...
`package.json`'s `"name"` must identify this template, not the boilerplate it was cloned from — see [`template-boilerplate-react`](https://github.com/dsplay/template-boilerplate-react)'s AGENTS.md for the full convention. This template's is `dsplay-template-voice-ai`.
...
- **Error handling**: unlike a plain REST call (see [`template-flight-information`](https://github.com/dsplay/template-flight-information)), `window.vapiSDK.run({ apiKey, assistant, config })` doesn't validate `assistant_id`/`api_key` upfront - the widget renders regardless, and a bad key only surfaces once a call is actually attempted. This means it can't be wired up as a `Loader` task/`tasksErrors` the way `template-flight-information` handles its API key - there's no promise to await during the intro. Instead, `src/components/vapi/index.jsx` listens for the `error`/`call-start-failed` events on the `vapiInstance` object returned by `.run()` and shows a translated message near the call button when either fires. Verified live: attempting a call with a bogus `assistant_id`/`api_key` no longer fails silently.
